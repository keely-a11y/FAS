...
- [ ] Crawl the old Wix sitemap.xml; enter every legacy project URL into Redirection, mapping to its new `/projects/{slug}/` equivalent. `/process-of-design` and `/about` are already handled in code.
...
- [ ] Axe: zero critical issues on every template (Home, Projects index, a project, Process, People, Purpose, Careers, Contact, Journal, 404).
